  ...
  if(n < num_of_con_req):
    ...
    for i in range(0, num_add):
      ...
      con = client.containers.run('acts', detach=True, ports={
                                  '8000/tcp': 8000 + k}, command="sh -c 'python manage.py collectstatic --noinput && sleep 2 && python manage.py makemigrations --noinput && python manage.py migrate --noinput && gunicorn --reload cloud_project.wsgi:application -b 0.0.0.0:8000 --error-logfile gunicorn.error'")
      print(con.logs())
